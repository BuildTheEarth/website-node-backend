...
export const checkUserPermission = (prisma: PrismaClient, permission: string, buildteam?: string) => {
...
export const checkUserPermissions = (prisma: PrismaClient, permissions: string[], buildteam?: string) => {
	return async (req: Request, res: Response, next: NextFunction) => {
		if (!req.kauth.grant) {
			ERROR_NO_PERMISSION(req, res);
			return;
